...
import { fetchAllGenere } from "@/store/slice/userSlicer"
...
const AllGenereList = () => {
  ...
  const [oldGenere, setOldGenere] = useState(null)
  const dispatch = useDispatch()
  const router = useRouter()
  const [loading, setLoading] = useState(true)
  const timeOutRef = useRef(null)
  const { AllGenere, status: genereStatus, error } = useSelector(
    (state) => state.userAllSlicer
  )

  useEffect(() => {
    if (session && authStatus === "authenticated") {
      dispatch(fetchAllGenere(session?.user?.jwtToken))
    }
  }, [session, authStatus, dispatch])

  useEffect(() => {
    setGenere(AllGenere)
    setOldGenere(AllGenere)
    setLoading(false)
  }, [genereStatus, AllGenere])
  ...
  const handleSearch = (searchValue) => {
    ...
    timeOutRef.current = setTimeout(() => {
      if (searchValue === "") {
        ...
        const filteredData = oldGenere.filter((item) =>
          item.name.toLowerCase().includes(searchValue)
        )
        setGenere(filteredData)
      }
    }, 500)
  }
  ...
  return (
    <Layout>
      <div className="container mt-4 mb-5">
        <h2 className="mb-4 fw-bold text-center">🎤 All Genres</h2>
        <div className="row">
          <div className="col-8 mx-auto">
            <input 
              ...
              placeholder="Enter Genre Name to Search ..."
            />
          </div>
        </div>
        <hr />
