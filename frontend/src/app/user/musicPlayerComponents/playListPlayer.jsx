...
const PlayListPlayer = ({data=[]}) => {
  ...
  const [songsBackUp, setSongBackUp] = useState(null)

  // Generate unique key for list items - memoize this function
  const generateUniqueKey = useCallback((id) => {
  ...
  // Create a new array without the selected song
  const filteredSongs = nextSongs.filter(item => item.id !== song.id)
  
  // Update the next songs queue
  setNextSongs(filteredSongs)
}, [nextSongs]) // Add nextSongs as dependency
