...
import { getAllStory, updateStory } from "@/app/utils/adminApi"
import { useSession } from "next-auth/react"
import Layout from "../common/layout"
import { deleteStory } from "@/app/utils/adminApi"
...
const AllStory = () => {
    ...
    const fetchData = async()=>{
        const res = await getAllStory({jwtToken:session?.user?.jwtToken})
        console.log("This is Response : ",res)
        if(res.code == 1){
            setStory(res.data)
            setOldStory(res.data)
        }
    }
    useEffect(()=>{
    ...
    const editButton = (rowData)=>{
        router.push(`/admin/editStory/${rowData.id}`)
    }
    ...
    const blockTemplate = (rowData) => {
        if(rowData.is_active){
            return <a icon="pi pi-bookmark" className="p-button-text" onClick={(e)=>{e.preventDefault();if(window.confirm('Are you sure you want to block this Song Story?')){blockButton(rowData)}}} ><i className="bi bi-check-circle adminUnblockBtn"></i></a>
        }else{
            return <a icon="pi pi-bookmark" className="p-button-text" onClick={(e)=>{e.preventDefault();if(window.confirm('Are you sure you want to unblock this Song Story?')){blockButton(rowData)}}} ><i className="bi bi-lock-fill adminBlockBtn"></i></a>
        }
    }
    const blockButton = async(rowData)=>{
        console.log("this is Blocked Data : ",rowData)
            let res = await updateStory({jwtToken:session?.user?.jwtToken, id:rowData.id, is_active:rowData.is_active,state:1})
            console.log("Errorrr : ",res)
            if(res.code == 1){
                rowData.is_active ? notify(<div><i className="bi bi-check-circle-fill text-success"></i> Story Blocked Successfully!</div>) : notify(<div><i className="bi bi-check-circle-fill text-success"></i> Story Unblocked Successfully!</div>)
            }else{
                rowData.is_active ? notify(<div><i className="bi bi-x-circle-fill text-danger"></i>Error in Blocking Story!</div>) : notify(<div><i className="bi bi-x-circle-fill text-danger"></i> Error in Unblocking Story!</div>)
            }

            const cat = story.map((item) => {
    ...
    const deleteTemplate = (rowData) => {
        return <a className="p-button-text" onClick={(e)=>{e.preventDefault();if(window.confirm('Are you sure you want to delete this Story?')){deleteButton(rowData)}}} ><i className="bi bi-trash adminDeletebtn"></i></a>
    }
    const deleteButton = async(rowData)=>{
        ...
        setOldStory(prevData => {
            const newData = prevData.filter(item => rowData.id != item.id);
            return newData
        })
        const res = await deleteStory({jwtToken : session?.user?.jwtToken, id:rowData.id})
        if(res?.code == 1){
            notify(<div><i className="bi bi-check-circle-fill text-success"></i> Story Deleted Successfully !</div>)
        }else{
            notify(<div><i className="bi bi-x-circle-fill text-danger"></i> Error in Deleting Story !</div>)
        }
    }
    ...
    const handleSearch = (searchValue) => {
        ...
        timeOutRef.current = setTimeout(() => {
            if (searchValue === "") {
                setStory(oldStory); // Reset to original data if search is cleared
            } else {
    ...
    const descriptionTemplate = (rowData) => {
        return <>{rowData.description.slice(0,64)}{rowData.description.length>64?" ...":""}</>
    }
    return (
        <Layout>
            <ToastContainer />
            {viewMoreBtn?<>
                ...
                <div className="card shadow-md p-4 bg-white rounded-lg adminForm">
                    <h2 className="text-xl font-semibold text-gray-700 mb-4 text-center">Song Stories</h2>
                    <div className="row">
                        ...
                        <div className="col-6 mb-3">
                            ...
                            <div className="h4 text-secondary">{viewMoreData?.star_name || "N/A"}</div>
                        </div>
                ...
                <>
                 ...
                 <h2 className="text-xl font-semibold text-gray-700 mb-4 text-center">All Stories</h2>
                 <div className="mb-3">
                     <input 
                     ref={timeOutRef}
                         type="text" 
                 ...
                 >
                     ...
                     <Column field="title" header="Title" style={{ width: '25%', padding: '0.5rem' }}></Column>
                     <Column field="star_name" header="Name" style={{ width: '25%', padding: '0.5rem' }}></Column>
